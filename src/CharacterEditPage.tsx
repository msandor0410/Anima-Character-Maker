...
export function CharacterEditPage() {
    ...
    useEffect(() => {
        ...
        getDoc(ref).then((snap) => {
            ...
            // Basics
            setName(data?.name ?? "");
            setLevel(typeof data?.level === "number" ? data.level : null);
            ...
            // Secondary
            if (data?.secondary?.skillBase) setSkillBase(data.secondary.skillBase);

            // Combat + LP
            const c = data?.combat ?? {};
            setCombatBase({
                attack: c?.attack?.base ?? 0,
                block: c?.block?.base ?? 0,
                dodge: c?.dodge?.base ?? 0,
                wearArmor: c?.wearArmor?.base ?? 0,
            });
            setLifePointMultiples(c?.lifePoints?.multiples ?? 0);

            // Advantages / Disadvantages
            if (Array.isArray(data?.advantages)) setSelectedAdvantages(data.advantages);
            if (Array.isArray(data?.disadvantages)) setSelectedDisadvantages(data.disadvantages);
        });
    }, [uid, id]);
    ...
    function setSkillBaseValue(skillId: string, nextBaseRaw: number) {
        setSkillBase((prev) => {
            ...
            // increase: clamp to remaining DP
    ...
    function setCombatBaseValue(id: CombatAbilityKey, nextBaseRaw: number) {
        setCombatBase((prev) => {
            ...
            // prev used combat includes wearArmor
    ...
    async function handleSave() {
        ...
        try {
            if (!id) return alert("Missing character id.");
                    ...
                    <div className="onboarding-sign-up">
                        <div className="div dashboard-card creator-card">
                            <div className="header-2">
                                <p className="p">Character Editor</p>
                            </div>
                            ...
                            <div className="creator-section">
                                ...
                                {locked && (
                                    <div className="mini warn">
                                        Choose a Level + Class-t for calculating the Development Points (DP) and to unlock editing sections.
                                    </div>
                                )}
                            ...
                            <fieldset disabled={locked} style={{ border: "none", padding: 0, margin: 0 }}>
                                <div className="creator-grid">
                                    ...
                                    <div className="creator-section">
                                        ...
                                        <div className="mini">Itt Base pontot növelsz 1-esével; a DP automatikusan levonódik.</div>
